...
def run_ssh_restart_command(
    hostname, username, password, command
):  # to execute a series of command which restart loopedge-dh and restore the cmd prompt
    ssh = (
        paramiko.SSHClient()
    )  # Create an instance of the SSHClient class from the Paramiko library to establish an SSH connection.
    ssh.set_missing_host_key_policy(
        paramiko.AutoAddPolicy()
    )  # Set the host key policy to AutoAddPolicy, which automatically adds unknown SSH server keys to the client's known hosts.
...
if __name__ == "__main__":
    ...
    commands_to_run = [
        ...
        "cd ~",
        "cd /var/lib/loopedge-analytics2",  # add rm -r commands too suitably
        "rm analytics2.db",
        "systemctl restart loopedge-analytics2",
        "cd ~",
        "cd /var/lib/loopedge-flows-manager",
        "rm flows-manager.db",
        "systemctl restart loopedge-flows-manager",
        "cd ~",
        "cd /var/lib/loopedge-marketplace",
        "rm marketplace.db",
        "systemctl restart loopedge-marketplace",
        "cd /var/lib/loopedge-cc",
        "rm cc.db",
        "systemctl restart loopedge-cc",
        "cd ~",
        "cd /var/lib/loopedge-opcua",
        "rm opcua.db",
        "systemctl restart loopedge-opcua",
        "cd ~",
    ]
